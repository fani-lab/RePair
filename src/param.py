...
settings = {
    'cmd': ["stat"],   # steps of pipeline, ['pair', 'finetune', 'predict', 'search', 'eval','agg', 'box','dense_retrieve','stat']
    'ncore': 8,
    ...
    'batch': 60,               # search per batch of queries for IR search using pyserini, if None, search per query
    ...
    'msmarco.doc': {
        'index_item': ['document'],    # ['url'], ['title', 'url'], ['title', 'url', 'text']
        'index': '../data/raw/toy.orcas/lucene-index.msmarco-v1-doc.20220131.9ea315', #' ../data/raw/robust/lucene-index.robust04.pos+docvectors+rawdocs',  # change based on index_item
        'dense_encoder': 'castorini/tct_colbert-v2-hnp-msmarco',  # change based on index_item
        'dense_index':'msmarco-doc-tct_colbert-v2-hnp-bf',  # change based on index_item
        ...
        'lseq': {"inputs": 32, "targets": 2048},  # query length and doc length for t5 model,
        'filter': {'minql': 1, 'mindocl': 10}   # [min query length, min doc length], after merge queries with relevant 'index_item', if |query| <= minql drop the row, if |'index_item'| < mindocl, drop row
    }
}
