...
class T5Transformer(AbstractQRefiner):
    def __init__(self, domain, corpus, ds, output):
        ...
        self.domain = domain
        self.output = output
        self.datapath = settings['datalist'][settings['domainlist'].index(self.domain)]
        self.prep_output = f'./../data/preprocessed/{os.path.split(self.datapath)[-1]}'
        self.model_dir = t5transformer['pretrained_dir']
        ...
        # self.tsv_path = {'train': f'{self.prep_output}/{self.ds.user_pairing}{self.in_type}.{self.out_type}.{self.index_item_str}.train.tsv', 'test': f'{self.prep_output}/{self.ds.user_pairing}{self.in_type}.{self.out_type}.{self.index_item_str}.test.tsv'}
