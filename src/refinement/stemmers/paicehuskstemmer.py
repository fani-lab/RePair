...
class PaiceHuskStemmer(object):
    ...
    def first_vowel(self, word):
        # Find the first position of each regular vowel (if any)
        has_vowel_list = [p for p in [word.find(v) for v in "aeiou"] if p > -1]
        # We add some logic to make sure y can be the only vowel
        if len(has_vowel_list) > 0:
            vp = min(has_vowel_list)
        else:
            vp = -1
        yp = word.find("y")
        if yp > 0 and (yp < vp | vp == -1):
            return yp
        return vp
            ...
            for ending, intact, num, append, cont in rulelist:
                if stem.endswith(ending):
                    ...
                    if ((pfv == 0 and newlen < 2)
                        ...
                        # 3 and there must be a vowel in the stem somewhere
                            continue
