...
class MsMarcoPsg(Dataset):
    ...
    def to_txt(pid):
        ...
        try: return json.loads(MsMarcoPsg.searcher.doc(str(pid)).raw())['contents'].lower()
    ...
    def to_pair(input, output, cat=True):
        ...
        queries_qrels[doccol] = queries_qrels['pid'].progress_apply(MsMarcoPsg.to_txt) #100%|██████████| 532761/532761 [00:32<00:00, 16448.77it/s]
        queries_qrels['ctx'] = ''
    ...
    def to_search(in_query, out_docids, qids, ranker='bm25', topk=100, batch=None):
        ...
        MsMarcoPsg.to_search_df(queries, out_docids, qids, ranker=ranker, topk=topk, batch=batch)
        ...
        with open(out_docids, 'w', encoding='utf-8') as o:
            if batch:
                for b in tqdm(range(0, len(queries), batch)):
                    ...
                    hits = Dataset.searcher.batch_search(queries.iloc[b: b + batch]['query'].values.tolist(), qids[b: b + batch], k=topk, threads=param.settings['ncore'])
    ...
    def box(input, qrels, output):
        ...
        for c in checks.keys():
            ...
            qrels.to_csv(f'{output}/{c}.qrels.tsv_', sep='\t', encoding='utf-8', index=False, header=False, columns=['qid', 'did', 'pid', 'rel'])
