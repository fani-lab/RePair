...
def to_txt(pid):
    ...
    global searcher
    try:
        if not searcher.doc(pid): return " "
        else: return json.loads(searcher.doc(str(pid)).raw())['contents'].lower()
    except Exception as e: raise e


def initiate_queries_qrels(input):
    """
    :input: location to store the files
    ...
    if not (isfile(join(input, 'queries.tsv'))):
        ...
        queries_df.drop(queries_df.loc[(len(queries_df['query'].str.strip()) <= param.settings['filter']['minql'])].index, inplace=True)
...
def to_pair(input, output, index_item, cat=True):
    global searcher
    searcher = LuceneSearcher(param.settings['aol']['index'] + index_item)
    if not searcher: raise ValueError(f'Lucene searcher cannot find/build aol index at {param.settings["aol"]["index"]}!')
    queries = pd.read_csv(f'{input}/queries.clean.tsv', sep='\t', index_col=False, names=['qid', 'query'], converters={'query': str.lower}, header=None)
    qrels = pd.read_csv(f'{input}/qrels.{index_item}.clean.tsv', encoding='UTF-8', sep='\t', index_col=False, names=['qid', 'iter', 'pid', 'relevancy'], usecols=['qid', 'pid', 'iter'], header=None)
