...
def getHits(input, output):
    ...
    try:
        ...
        for iter in predicted_file_list:
            ...
            with open(f'{output}runs/msmarco/passage_run{iter.split(".")[0][-3:]}.txt', 'w', encoding='utf-8') as retrieved_passage:
                print(f"getting relevant passages for {predicted_queries.shape[0]} queries for {iter.split('.')[0]}\n")
...
# def compute_metrics(metrics_input,metrics_output):
